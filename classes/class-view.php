<?php
/**
 * Members and methods for WTG Portal Manager views that use post boxes (eventually all views)
 *
...
/**
 * WTG Portal Manager Base View class
 * 
 * Taking from WTGPORTALMANAGER plugin and customized to pave the way for postboxes on multiple
 ...
 * @version 1.0.2
 */
abstract class WTGPORTALMANAGER_View {
     ...
     * @since 0.0.1
     *
     * @var WTGPORTALMANAGER_Admin_Page
     */
    ...
    public function __construct() {
        ...
        $this->UI = $this->WTGPORTALMANAGER->load_class( 'WTGPORTALMANAGER_ADMINUI', 'class-adminui.php', 'classes' );
        ...
        if( isset( $this->help_array[ $page_name ] ) ){
            ...
            if( isset( $this->help_array[ $page_name ][ 'pageinfo' ][ 'pageabout' ] ) ){
                ...
                $screen->add_help_tab( array(
                    'id' => $page_name,
                    'title' => __( 'About', 'wtgportalmanager' ) . ' ' . $this->help_array[ $page_name ][ 'pageinfo' ][ 'pagetitle' ],
                ...
                <p><strong>' . __( 'For more information:', 'wtgportalmanager' ) . '</strong></p>' . $pagereadmeurl . $pagevideourl . $pagediscussionurl . $pagefaqurl );
    ...
    */
    public function setup( $action, array $data ) {
        ...
        $this->WTGPORTALMANAGER->enqueue_script( 'common', array( 'jquery', 'postbox' ), array() );
    ...
    */
    protected function add_text_box( $id, $callback, $context = 'normal', $wrap = false ) {
        ...
        $long_id = "wtgportalmanager_{$this->action}-{$id}";
    ...
    */
    protected function add_meta_box( $id, $title, $callback, $context = 'normal', $priority = 'default', $callback_args = null ) {
        ...
        $id = "wtgportalmanager_{$this->action}-{$id}";         
    ...
    */
    protected function action_nonce_field( array $data, array $box ) {
        wp_nonce_field( $this->WTGPORTALMANAGER->nonce( $this->action ) ); echo "\n";
    }
    ...
    */
    protected function action_field( array $data, array $box ) {
        echo "<input type=\"hidden\" name=\"action\" value=\"wtgportalmanager_{$this->action}\" />\n";
    }
    ...
    */       
    public function render() { 
        ...
        if( !isset( $this->menu_array[ $admin_page ]['tabmenu'] ) || $this->menu_array[ $admin_page ]['tabmenu'] === true ) {
            if( $admin_page !== 'main' ) {
                $this->WTGPORTALMANAGER->build_tab_menu( $admin_page );
            }
        }
            ...
            <div id="poststuff">
                <div id="post-body" class="metabox-holder columns-<?php echo ( isset( $GLOBALS['screen_layout_columns'] ) && ( 2 == $GLOBALS['screen_layout_columns'] ) ) ? '2' : '1'; ?>">
                     ...
                     <div id="postbox-container-2" class="postbox-container">
                        <?php
                        $this->do_text_boxes( 'normal' );// see function add_text_box()
    ...
    */
    protected function textbox_submit_button( array $data, array $box ) {
        $caption = isset( $data['submit_button_caption'] ) ? $data['submit_button_caption'] : __( 'Save Changes', 'wtgportalmanager' );
        ?>
