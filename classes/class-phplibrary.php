...
//class WTGPORTALMANAGER_PHP extends WTGPORTALMANAGER_Flags { commented 14092014
class WTGPORTALMANAGER_PHP {  
    /**
    * Performs a var_dump if debug_mode active (debug_mode is only 
    * active when on installation blog unless manually activated)
    * 
    * @version 1.3
    */
    public function var_dump( $value, $header = false, $append = '<br>' ){
        if( defined( 'WEBTECHGLOBAL_ERRORDISPLAY' ) && WEBTECHGLOBAL_ERRORDISPLAY === true ){
    ...
    */
    ...
    /**
    * Compares giving possible/expected prepend (needle) characters to 
    * haystack string and determines if the
    ...
    */
    public function arrayinfo_set( $array, $line, $function, $file ){
        $array['arrayinfo']['version'] = WTGPORTALMANAGER_VERSION;
        $array['arrayinfo']['line'] = $line;
        $array['arrayinfo']['function'] = $function;
        $array['arrayinfo']['file'] = $file;
        $array['arrayinfo']['date'] = WTGPORTALMANAGER::datewp();
        return $array;   
    }  
    ...
    * 2. Faster than wtgportalmanager_count_extension_in_directory() because it returns sooner, important for directory with a lot of files
    * 
    * @returns boolean
    */
    ...
    */
    public function type( $value ){
        if( WTGPORTALMANAGER::is_image_url( $value ) ){
            return 'image';
        }
        
        // so its not an image url, is it any type of url? either for using as link or PHP functionality
        if( WTGPORTALMANAGER::is_url( $value ) ){
            return 'url';    
        }
        
        // check if decimal before checking if a plain number or money
        if( WTGPORTALMANAGER::is_decimalnumber( $value ) ){
            return 'decimal';
        }

        // is money value
        if( WTGPORTALMANAGER::is_string_money( $value ) ){
            return 'money'; 
        }
        ...
        // is email address
        if( WTGPORTALMANAGER::is_valid_emailaddress( $value ) ){
            return 'email';
        }                  
    ...
    */
    public function months_time_range( $month, $year, $return = 'array', $format = 'time' ){
        ...
        if( $format == 'date' )
        {
            $array['start'] = WTGPORTALMANAGER::datewp(0, $array['start'] );
        }
        ...
        if( $format == 'date' )
        {    
            $array['end'] = WTGPORTALMANAGER::datewp(0, $array['end'] );   
        }
...
}// end class WTGPORTALMANAGER_PHP
?>
