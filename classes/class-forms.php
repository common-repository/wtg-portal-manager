 ...
 * @version 1.6
 */  
class WTGPORTALMANAGER_Formbuilder extends WTGPORTALMANAGER_ADMINUI {
    ...
    var $input_types = array( 
        ...
        'menu_categories',
        'menu_users',
        'checkboxes',// depreciated
        'checkboxessingle',// name and ID incremented
        'checkboxesgrouped',// array submitted
        'menu',
        'textarea',
        'radiogroup',
        'switch',
    ...
    public function __construct() {
        ...
        $this->WPCore = WTGPORTALMANAGER::load_class( 'WTGPORTALMANAGER_WPCore', 'class-wpcore.php', 'classes' );
        $this->HELP = WTGPORTALMANAGER::load_class( 'WTGPORTALMANAGER_Help', 'class-help.php', 'classes' );
        $this->DB = WTGPORTALMANAGER::load_class( 'WTGPORTALMANAGER_DB', 'class-wpdb.php', 'classes' );
        
        // get form validation array
        $this->form_val_arr = get_option( 'wtgportalmanager_formvalidation' );
        
        // re-populate class var with the localized value
        $this->first_switch_label = __( 'Enabled', 'wtgportalmanager' );
        $this->second_switch_label = __( 'Disabled', 'wtgportalmanager' );
    }
    ...
    */
    ...
    /**
    * Returns array of validation types or if $string used 
    * confirms if a validation type is valid.
    * 
    * @author Ryan R. Bayne
    * @package WebTechGlobal WordPress Plugins
    * @since 0.0.1
    ...
    */
    ...
    /**
    * Returns array of sanitization methods or if $string used 
    * confirms if a sanitization method is valid.
    * 
    * @author Ryan R. Bayne
    * @package WebTechGlobal WordPress Plugins
    * @since 0.0.1
    * @version 1.0
    */
    public function sanitization_types( $string = null ) {
        if( !$string ) { return $this->santization_types; }
        if( in_array( $string, $this->santization_types ) ) {
            return true;
        } else {
            return false;
        }    
    ...
    * @param mixed $titleatt
    * @param mixed $required
    * @param mixed $currentvalue
    * @param mixed $atts_array can be used to configure an input
    * @param mixed $validation_array
    * @param mixed $sanitization_array
    */
    public function input( $formid, $inputtype, $inputid, $inputname, $optiontitle, $titleatt = '', $required = false, $currentvalue = null, $atts_array = array(), $validation_array = array(), $sanitization_array = array() ) {
    ...
    * 
    * Every form should have a unique ID within the entire plugin and not just
    * the page. This is to make it easier for developing help, support, tracking
    * and other systems that require a trace or history.
    * 
    * @author Ryan R. Bayne
    * @package WebTechGlobal WordPress Plugins
    * @since 0.0.1
    * @version 1.0
    */
    public function register_form( $form_id, $postbox_id, $page_name ) {
        global $wpdb;
        
        // registers the form on a per user basis as part of WTG Security for WordPress
        self::register_form_forsecurity( $form_id, $postbox_id, $page_name );
    }
    
    /**
    * Register the form on a per user basis as part of security.
    * 
    * @author Ryan R. Bayne
    * @package WebTechGlobal WordPress Plugins
    * @since 0.0.9
    * @version 1.0
    */
    public function register_form_forsecurity( $form_id, $postbox_id, $page_name ) {
        // initiate array
        ...
        update_option( 'wtgportalmanager_formvalidation', $this->form_val_arr );
    }
    ...
    */
        ...
        foreach( $this->form_val_arr[ $user_ID ][ $formid ] as $input_name => $input_array ) { 
            ...
            if( isset( $input_array['required']) && $input_array['required'] === true ) {
                ...
                if( $input_array['inputtype'] === 'checkboxes' ) {
                    ...
                    // array in $_POST - ensure checkbox array exists (user may not have made any selection)
    ...
    /**
    * Sanitize values after validation as I do not want to change users input
    * prior to validation and would prefer validation to throw an error if
    * sanitization results in an invalid value.
    * 
    * @author Ryan R. Bayne
    * @package WebTechGlobal WordPress Plugins
    * @since 0.0.1
    * @version 1.1
    */
    public function apply_input_sanitization( $formid ) { 
    
        // require user ID to obtain data about the form
        $user_ID = get_current_user_id();
        ...
        // if no sanitization array return a true result now to avoid failure
        if( !$this->form_val_arr || !$this->form_val_arr[ $user_ID ][ $formid ] ) {
            return $result;
        }                                               
        ...
        foreach( $_POST as $name => $value ) {
            ...
            // main validation - get the inputs validation method if it has been registered
            //$validation_methods_array = self::get_input_validation_methods_array( $formid, $name );
            $sanitization_methods_array = false;
            if( isset( $this->form_val_arr[ $user_ID ][ $formid ][ $name ]['sanitizationarray'] ) ) { 
                $sanitization_methods_array = $this->form_val_arr[ $user_ID ][ $formid ][ $name ]['sanitizationarray'];
            }
                    
            if( $sanitization_methods_array !== false && is_array( $sanitization_methods_array ) ) {
                
                // loop through each validation method, calling its function
                foreach( $sanitization_methods_array as $sanitization_method => $atts_mixed ) { 
                                                          
                    // build attributes array i.e. if maximum length validation, pass ['maxlength']
                    $atts_array = array();// not needed for all validation
                                        
                    // ensure the expected input_validation_example() function exists
                    if( method_exists( $this, 'sanitize_' . $sanitization_method ) ) { 
                                  
                        // input_validation functions return boolean false on fail and output a notice to make user aware
                        // else nothing else happens, next input is processed
                        eval( '$result = self::validate_' . $sanitization_method . '( $value, $this->form_val_arr[ $user_ID ][ $formid ][ $name ][ "optiontitle" ], $atts_mixed );');// $result is always boolean    
    ...
    * i.e. if values in a menu include Red, Green Blue and the visitor edits the menu to
    * include Yellow. Then this would be caught on submission and instantly rejected.
    * 
    * @author Ryan R. Bayne
    * @package WebTechGlobal WordPress Plugins
    * @since 0.0.1
    ...
    */
    public function apply_input_security() {
        ...
        if( !isset( $_POST['wtgportalmanager_form_formid'] ) ) { 
            return false;
        }
        
        $form_id = $_POST['wtgportalmanager_form_formid'];
            ...
            foreach( $this->form_val_arr[ $user_ID ][ $form_id ] as $input_name => $input_array ) {
                ...
                if( $input_array['inputtype'] == 'menu' ) {
                    ...
                    if( $found_match === false ) {
                        $this->UI->create_notice( __( 'CSV 2 POST has security related concerns about a menu on the form you submitted. Please discontinue your action and contact support. This event has been logged for investigation.', 'wtgportalmanager' ), 'warning', 'Small', __( 'Warning', 'wtgportalmanager' ) );                    
                        return false;    
                    }
                }                
                ...
                if( $input_array['inputtype'] == 'radiogroup' ) {
                    ...
                    // if no match, the submitted item value was not registered when the form was parsed
                    if( $found_match === false ) {
                        $this->UI->create_notice( __( 'CSV 2 POST detected a problem and could not continue. Please try again and if it continues contact support.', 'wtgportalmanager' ), 'warning', 'Small', __( 'Warning', 'wtgportalmanager' ) );                    
                        return false;    
                    }
                }               
                    ...
                    if( !isset( $_POST[ $input_name ] ) ) {
                       // arriving here indicates hidden input missing, a hack has removed it
                        ...
                        if( $input_array['currentvalue'] !== $_POST[ $input_name ] ) {
                            $this->UI->create_notice( __( 'CSV 2 POST has detected a possible hack attempt. Your activity has been logged. Please discontinue your action and contact support if this message continues to appear without any known reason.', 'wtgportalmanager' ), 'warning', 'Small', __( 'Warning', 'wtgportalmanager' ) );                    
                            return false;                            
                        }
                    }
                }   
    ...
    */    
    public function user_can( $capability = 'activate_plugins' ){
        if(!current_user_can( $capability ) ){
            $this->UI->create_notice( __( 'Your request was rejected as your account does not have the required permission.', 'wtgportalmanager' ), 'success', 'Small', __( 'Not Permitted', 'wtgportalmanager' ) );          
            return false;
        }   
        return true;
    }     
    ...
    */
    public function form_start( $mainform_id, $form_name, $form_title, $uploader = false ){

        // escape
        $form_name = esc_attr( strtolower( $form_name ) ); 
        $mainform_id = esc_attr( strtolower( $mainform_id ) );
        $form_title = esc_attr( $form_title );        
        $page_name = WTGPORTALMANAGER::get_admin_page_name();
        
        // re-create postbox ID - always based on plugin slug, page name and ID of main form.
        $postbox_id = 'wtgportalmanager_' . $page_name . '-' . $page_name . '-' . $mainform_id; 
   
        // register the form 
        self::register_form( $mainform_id, $postbox_id, $page_name );
        
        $action = $this->PHP->currenturl();
        
        $enctype = '';
        if( $uploader ) {
            $enctype = esc_attr( ' enctype="multipart/form-data"' );
        }
        
        // begin building the start of form and add standard hidden inputs
        echo '<form' . $enctype . ' method="post" name="' . $form_name . '" id="' . $mainform_id . '" action="' . $action . '">';
        ...
        echo '<input type="hidden" name="wtgportalmanager_hidden_pagename" value="' . $page_name . '">';
        ...
        echo '<input type="hidden" name="wtgportalmanager_postbox_id" value="' . $postbox_id . '">';   
        
        // display developer environment tools
        if( get_option( 'webtechglobal_helpauthoring' ) ) {
            self::help_content_authoring_postboxfields( $mainform_id, $form_name );
        }     
    }

    /**
    * Help content authoring form. Will only show in developer mode and
    * allows in-context authoring of help content. 
    * 
    * This approach is taking so that form attributes are easily captured
    * along with the help content.
    * 
    * @author Ryan R. Bayne
    * @package WebTechGlobal WordPress Plugins
    * @since 0.0.1
    * @version 1.0
    */
    public function help_content_authoring_postboxfields( $form_id, $form_name ) {
        global $wtgportalmanager_settings;
 
        // require the unique dev mode id, used to quickly
        // fix id conflicts with new plugins
        if(!defined( "WTGPORTALMANAGER_DEVMODE_ID" ) ){
            return;
        }

        if( !isset( $wtgportalmanager_settings['developermode']['developermodeswitch'] ) 
        || $wtgportalmanager_settings['developermode']['developermodeswitch'] !== 'enabled' ) {
            return;
        }

        // lets ensure only admin get access to this form
        ...
        // get existing help data
        $help_array = $this->HELP->get_help_array();
        
        echo '<table class="form-table">';
        
        self::input_subline( __( 'Developer Mode Active: Help Content Fields Displayed', 'wtgportalmanager' ), '' );
        
        // set input attribute values
        $id = esc_attr( $form_id ) . '_boxintro_' . WTGPORTALMANAGER_DEVMODE_ID;
        $name = esc_attr( $form_id ) . '_boxintro_' . WTGPORTALMANAGER_DEVMODE_ID;
        
        // get current help content, if any
        $current_value = '';
        
        if( isset( $help_array[ $page_name ]['forms'][ $form_id ][ 'formabout' ] ) ) {
            $current_value = $help_array[ $page_name ]['forms'][ $form_id ][ 'formabout' ];
        } 
        ?>
        ...
        <tr valign="top">
            ...
            <td>
                <textarea id="<?php echo esc_attr( $id );?>" name="<?php echo esc_attr( $name ); ?>" rows="10" cols="50"><?php echo esc_textarea( $current_value );?></textarea>
            </td>
        </tr>
        <!-- Option End --> 
        ...
        <?php 
        // set input attribute values
        $id = esc_attr( $form_id ) . '_readmoreurl_' . WTGPORTALMANAGER_DEVMODE_ID;
        $name = esc_attr( $form_id ) . '_readmoreurl_' . WTGPORTALMANAGER_DEVMODE_ID;
        $current_value = '';        
        ?>        
        <!-- Option Start -->        
        <tr valign="top">
            ...
            <td>
                <input type="url" id="<?php echo esc_attr( $id );?>" name="<?php echo esc_attr( $name ); ?> value="<?php echo esc_html( $current_value ); ?>" size="50">
            </td>
        </tr>
        <!-- Option End -->
        ...
        <?php 
        // set input attribute values
        $id = esc_attr( $form_id ) . '_formvideourl_' . WTGPORTALMANAGER_DEVMODE_ID;
        $name = esc_attr( $form_id ) . '_formvideourl_' . WTGPORTALMANAGER_DEVMODE_ID;
        $current_value = '';        
        ?>        
        <!-- Option Start -->        
        <tr valign="top">
        ...
        <?php 
        // set input attribute values
        $id = esc_attr( $form_id ) . '_formvideoid_' . WTGPORTALMANAGER_DEVMODE_ID;
        $name = esc_attr( $form_id ) . '_formvideoid_' . WTGPORTALMANAGER_DEVMODE_ID;
        $current_value = '';        
        ?>        
        <!-- Option Start -->        
        <tr valign="top">
            ...
            <td>
                <input type="text" id="<?php echo esc_attr( $id );?>" name="<?php echo esc_attr( $name ); ?> value="<?php echo esc_html( $current_value ); ?>" size="50">
            </td>
        </tr>
        <!-- Option End -->
        ...
        <?php 
        // set input attribute values
        $id = esc_attr( $form_id ) . '_formdiscussurl_' . WTGPORTALMANAGER_DEVMODE_ID;
        $name = esc_attr( $form_id ) . '_formdiscussurl_' . WTGPORTALMANAGER_DEVMODE_ID;
        $current_value = '';        
        ?>        
        <!-- Option Start -->        
        <tr valign="top">
        ...
        <?php 
        // set input attribute values
        $id = esc_attr( $form_id ) . '_helpurl_' . WTGPORTALMANAGER_DEVMODE_ID;
        $name = esc_attr( $form_id ) . '_helpurl_' . WTGPORTALMANAGER_DEVMODE_ID;
        $current_value = '';        
        ?>        
        <!-- Option Start -->        
        <tr valign="top">
    ...
    /**
    * Help content authoring for individual form inputs. 
    * 
    * Will only show in developer mode and
    * allows in-context authoring of help content. 
    * 
    * This approach is taking so that form attributes are easily captured
    * along with the help content.
    ...
    * @version 1.0
    */
    public function help_content_authoring_fields_input( $input_id, $input_name, $form_id ) {
        global $wtgportalmanager_settings;
        ...
        // set input attribute values, form ID to make unique on page, 
        $id = esc_attr( $this->inputid ) . $form_id . '_aboutinput_' . WTGPORTALMANAGER_DEVMODE_ID;
        $name = esc_attr( $this->inputname ) . $form_id . '_aboutinput_' . WTGPORTALMANAGER_DEVMODE_ID;
        $current_value = '';
        ?>
        ...
        <tr valign="top">
            ...
            <td>
                <textarea id="<?php echo $id;?>" name="<?php echo $name; ?>" rows="10" cols="50"><?php echo esc_textarea( $current_value );?></textarea>
            </td>
        </tr>
        <!-- Option End -->
    ...
    */
    public function validate_alpha( $value, $optiontitle, $atts_array ) {
        if( !ctype_alpha( $value ) ) {
            $this->UI->create_notice( sprintf( __( "The %s input is only for letters, please edit your entry and remove any numbers or special characters.", 'wtgportalmanager' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'wtgportalmanager' ) );               
            return false;            
        }
        return true;
    }
    ...
    */
    public function validate_alphanumeric( $value, $optiontitle ) {
        if( !ctype_alnum( $value ) ) {   
            $this->UI->create_notice( sprintf( __( "The %s input is for letters or numbers, no special characters allowed.", 'wtgportalmanager' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'wtgportalmanager' ) );               
            return false;
        }
        return true;
    }
    ...
    */
    public function validate_url( $url, $optiontitle ) {    
        if( !$this->PHP->validate_url( $url ) ) {
            $this->UI->create_notice(sprintf( __( "You did not enter a valid URL in the %s input. Please ensure that all characters match the online URL.", 'wtgportalmanager' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'wtgportalmanager' ) );               
            return false;
        }        
        return true;
    } 
    ...
    */
    public function validate_numeric( $value, $optiontitle ) {  
        if( !ctype_digit( $value ) ) {
            $this->UI->create_notice( sprintf( __( "Only numbers are meant to be entered into the %s field.", 'wtgportalmanager' ), $optiontitle ), 'error', 'Small', __( 'Invalid Entry', 'wtgportalmanager' ) );               
            return false;
        }        
        return true;    
    }
    ...
    */
    public function validate_maxlength( $str, $optiontitle, $atts_mixed ) {
        if ( strlen( $str ) > $atts_mixed ) {
            $this->UI->create_notice( sprintf( __( "The maximum length for the %s field is %s ", 'wtgportalmanager' ), $optiontitle, $atts_mixed ), 'error', 'Small', __( 'Too Long', 'wtgportalmanager' ) );               
            return false;
        }        
        return true;        
    }    
    ...
    */
    public function validate_minlength( $str, $optiontitle, $atts_mixed ) {
        if ( strlen( $str ) < $atts_mixed ) {
            $this->UI->create_notice( sprintf( __( "The minimum length for the %s field is %s ", 'wtgportalmanager' ), $optiontitle, $atts_mixed ), 'error', 'Small', __( 'Too Short', 'wtgportalmanager' ) );               
            return false;
        }        
        return true;        
    }
    ...
    */
    public function validate_regex( $str, $optiontitle, $atts_mixed ) {
        if ( !preg_match( $atts_mixed[0], $str ) ) {
            $this->UI->create_notice( sprintf( __( "The %s field entry is not the correct format. A specific pattern of characters is required i.e. %s.", 'wtgportalmanager' ), $optiontitle, $atts_mixed[1] ), 'error', 'Small', __( 'Invalid Value', 'wtgportalmanager' ) );               
            return false;
        }        
        return true;    
    }
    ...
    * @version 1.0
    */
    public function sanitize_email( $str, $optiontitle, $atts_mixed ) {
    ...
    * @version 1.0
    */
    public function sanitize_file_name( $str, $optiontitle, $atts_mixed ) {
    ...
    * @version 1.0
    */
    public function sanitize_key( $str, $optiontitle, $atts_mixed ) {
    ...
    * @version 1.0
    */
    public function sanitize_option( $str, $optiontitle, $atts_mixed ) {
        return sanitize_option( $optiontitle, $str );    
    }
    ...
    * @version 1.0
    */
    public function sanitize_text_field( $str, $optiontitle, $atts_mixed ) {
    ...
    * @version 1.0
    */
    public function sanitize_title( $str, $optiontitle, $atts_mixed ) {
        return sanitize_title( $str );// ~ fallback ~ context   
    }
    ...
    * @version 1.0
    */
    public function sanitize_user( $str, $optiontitle, $atts_mixed ) {
        return sanitize_user( $str, false );   
    }

    /**
    * Santize the name of a user role. Do not confuse with the readable
    * title of a role.
    * 
    * @author Ryan R. Bayne
    * @package WebTechGlobal WordPress Plugins
    ...
    * @version 1.0
    */
    public function sanitize_userrolename( $str, $optiontitle, $atts_mixed ) {
        // sanitize_key() should do the job for this
        return sanitize_key( $str );    
    }    
    ##########################################################
    #                                                        #
    ...
    */
    public function input_subline( $secondfield, $firstfield = '' ){?>
        <!-- Option Start -->        
        <tr valign="top">
            <th scope="row"><?php echo esc_html( $firstfield );?></th>
            <td><?php echo esc_html( $secondfield );?></td>
        </tr>
        <!-- Option End --><?php     
    } 
    ...
    */
    public function input_text(){
        ...
        if( isset( $this->appendtext ) && $this->appendtext === true ){ $appendtext = esc_html( $this->appendtext ); }
        ?>
        <!-- Option Start -->        
        <tr valign="top">
            <th scope="row">
    ...
    /**
    * Add password input to WordPress style form which is tabled and 
    * has optional HTML5 support.
    * 
    * @param string $title
    * @param string $name - html name
    * @param string $id - html id
    ...
    * @param boolean $required
    * 
    * @param string $validation - appends 'input_validation_' to call a function 
    * that validates, so any function and validation method can be setup 
    *      
    */
    public function input_password(){
        ...
        <tr valign="top">
            ...
            <td>
                ...
                <input type="password" name="<?php echo $this->inputname;?>" id="<?php echo $this->inputid;?>" value="<?php echo $this->currentvalue;?>" title="<?php echo $this->titleatt; ?>"<?php echo $readonly;?><?php echo $this->required;?><?php echo $disabled; ?>> 
    ...
    */
    public function input_switch(){
        ...
        <!-- Option Start -->
        <tr valign="top">
            <th scope="row"><?php _e( $this->optiontitle, 'wtgportalmanager' ); ?></th>
            <td>
                <fieldset<?php echo esc_attr( $disabled ); ?>><legend class="screen-reader-text"><span><?php echo esc_html( $this->optiontitle ); ?></span></legend>
                    <input type="radio" id="<?php echo esc_attr( $this->inputname );?>_enabled" name="<?php echo esc_attr( $this->inputname );?>" value="enabled" <?php echo self::is_checked( $this->currentvalue, 'enabled', 'return' );?> />
                    <label for="<?php echo esc_attr( $this->inputname );?>_enabled"> <?php echo esc_html( $this->first_switch_label ); ?></label>
                    <br />
                    <input type="radio" id="<?php echo esc_attr( $this->inputname );?>_disabled" name="<?php echo esc_attr( $this->inputname );?>" value="disabled" <?php echo self::is_checked( $this->currentvalue, 'disabled', 'return' );?> />
    ...
    */
    public function input_radiogroup(){
        ...
        <!-- Option Start -->        
        <tr valign="top">
            <th scope="row">' . esc_html( $this->optiontitle ) . '</th>
            <td><fieldset>';
            
            echo '<legend class="screen-reader-text"><span>' . esc_html( $this->optiontitle ) . '</span></legend>';
            ...
            foreach( $this->itemsarray as $item_value => $item_title ){
                ...
                <input type="radio" id="<?php echo esc_attr( $this->inputid ) . $i; ?>" name="<?php echo esc_attr( $this->inputid ); ?>" value="<?php echo esc_attr( $item_value ); ?>"<?php echo $checked;?><?php if( isset( $this->disabled ) && $this->disabled === true ){ echo ' disabled';} ?>/>
                <label for="<?php echo esc_attr( $this->inputid ) . $i; ?>"><?php echo esc_html( $item_title ); ?></label>
                <br />
    ...
    public function input_textarea(){
        ...
        <tr valign="top">
            <th scope="row">
                ...
                <?php echo esc_html( $this->optiontitle );
            ...
            <td>
                <textarea id="<?php echo esc_attr( $this->inputid );?>" name="<?php echo esc_attr( $this->inputname );?>" rows="<?php echo esc_attr( $this->rows );?>" cols="<?php echo esc_attr( $this->cols );?>"<?php if( isset( $this->required ) && $this->required === true ){echo ' required';}?><?php if( isset( $this->disabled ) && $this->disabled === true ){ echo ' disabled';} ?>><?php echo esc_textarea( $this->currentvalue );?></textarea>
            </td>
        </tr>
        <!-- Option End -->
        <?php     
    ...
    */
    public function input_menu(){
        ...
        <!-- Option Start -->        
        <tr valign="top">
            <th scope="row"><label for="<?php echo esc_attr( $this->inputid ); ?>"><?php echo esc_html( $this->optiontitle ); ?></label></th>
            <td>            
                <select name="<?php echo esc_attr( $this->inputname );?>" id="<?php echo esc_attr( $this->inputid );?>"<?php if( isset( $this->disabled ) && $this->disabled === true ){ echo ' disabled'; } ?>>
                    <?php
                    ...
                    if( isset( $this->defaultitem_name ) || isset( $this->defaultitem_value ) ) {
                        ...
                        echo '<option selected="selected" value="' . esc_attr( $def_item_value ) . '">' . esc_html( $def_item_name ). '</option>';
                    }
                    ...
                    foreach( $this->itemsarray as $key => $option_title ){
                        ...
                        echo '<option ' . esc_attr( $selected ) . ' value="' . esc_attr( $key ) . '">' . esc_html( $option_title ) . '</option>';    
                    }
                    ?>
                </select>                  
            </td>
    ...
    * @version 1.1
    * 
    * @deprecated use input_checkboxessingle() or input_checkboxesgrouped()
    */
    public function input_checkboxes(){
        ...
        <!-- Option Start -->
        <tr valign="top">
            <th scope="row"><?php echo esc_html( $this->optiontitle );?></th>
            <td>
            ...
            foreach( $this->itemsarray as $item_value => $item_title ) {
                ...
                echo '<label for="' . esc_attr( $this->inputid ) . $i . '"><input type="checkbox" name="' . esc_attr( $inputname ) . '" id="' . esc_attr( $this->inputid ) . $i . '" value="' . esc_attr( $item_value ) . '"' .  $checked . $disabled . '>' . esc_html( $item_title ) . '</label>';
                $first = false;
                ++$i;
            }
            ?>
    ...
    * @version 1.1
    */
    public function input_checkboxessingle(){
        ...
        <tr valign="top">
            ...
            foreach( $this->itemsarray as $item_value => $item_title ) {
                ...
                $inputname = esc_attr( $this->inputid ) . $i;
      
                echo '<label for="' . esc_attr( $this->inputid ) . $i . '"><input type="checkbox" name="' . $inputname . '" id="' . esc_attr( $this->inputid ) . $i . '" value="' . esc_attr( $item_value ) . '"' .  $checked . $disabled . '>' . esc_html( $item_title ) . '</label>';
                $first = false;
                ++$i;
            }
            ?>
    ...
    * @version 1.1
    */
    public function input_checkboxesgrouped(){
        ...
        <tr valign="top">
            ...
            foreach( $this->itemsarray as $item_value => $item_title ) {
                ...
                echo '<label for="' . esc_attr( $this->inputid ) . $i . '"><input type="checkbox" name="' . esc_attr( $this->inputname ) . '[]" id="' . esc_attr( $this->inputid ) . $i . '" value="' . esc_attr( $item_value ) . '"' .  $checked . $disabled . '>' . esc_html( $item_title ) . '</label>';
                $first = false;
                ++$i;
            }
            ?>
    ...
    */
    public function input_menu_users(){
        ...
        <tr valign="top">
            <th scope="row"><?php echo esc_html( $this->optiontitle );?></th>
            <td>                                     
                <select name="<?php echo esc_attr( $this->inputname );?>" id="<?php echo esc_attr( $this->inputid );?>"<?php if( $this->disabled ){ echo ' disabled';} ?>>
            
                    <?php        
                    $selected = '';                    
                    ?>
                    
                    <option value="notselected" <?php echo $selected;?>><?php _e( 'None Selected', 'wtgportalmanager' ); ?></option> 
                    ...
                    foreach ( $blogusers as $user ){ 
                        ...
                        echo '<option value="' . esc_attr( $user->ID ) . '" ' . esc_attr( $selected ) . '>' . esc_html( $user->ID ) . ' - ' . esc_html( $user->display_name ) . '</option>'; 
                    }?>
            ...
            <td>                                     
                <select name="<?php echo esc_attr( $this->inputname );?>" id="<?php echo esc_attr( $this->inputid );?>"<?php if( $this->disabled ){ echo ' disabled';} ?>>
                    ...
                    foreach( $cats as $c ){ 
                        ...
                        echo '<option value="' . esc_attr( $c->term_id ) . '" ' . esc_attr( $selected ) . '>' . esc_html( $c->term_id ) . ' - ' . esc_html( $c->name ) .'</option>'; 
                    }?>
        ...
        <tr valign="top">
            ...
            foreach( $post_types as $post_type ){
                ...
                if( $post_type != 'post' ){
                    ...
                    echo '<input type="radio" name="' . esc_attr( $this->inputname ) . '" id="' . esc_attr( $this->inputid ) . $i . '" value="' . esc_attr( $post_type ) . '" ' . $checked . ' />
                    <label for="' . esc_attr( $this->inputid ) . $i . '"> ' . esc_html( $post_type ) . '</label><br>';
            ...
            echo '<input type="radio" name="' . esc_attr( $this->inputname ) . '" id="' . esc_attr( $this->inputid ) . $i . '" value="post" ' . esc_attr( $post_default ) . ' />
            <label for="' . esc_attr( $this->inputid ) . $i . '">post</label>';
        ...
        <tr valign="top">
            ...
            if ( is_array( $post_formats[0] ) ) {
                ...
                foreach( $post_formats[0] as $key => $format ){
                    ...
                    echo '<input type="radio" id="' . esc_attr( $this->inputid ) . $i . '" name="' . esc_attr( $this->inputname ) . '" value="' . esc_attr( $format ) . '"' . $statuschecked . '/>
                    <label for="' . esc_attr( $this->inputid ) . $i . '">' . esc_html( $format ) . '</label><br>';
                    ++$i; 
                }
                
                if(!$optionchecked){$statuschecked = ' checked="checked" ';}
                
                echo '<input type="radio" id="' . esc_attr( $this->inputid ) . $i . '" name="' . esc_attr( $this->inputname ) . '" value="standard"' . $statuschecked . '/>
                <label for="' . esc_attr( $this->inputid ) . $i . '">standard (default)</label>';               
    ...
    */
    public function input_file(){?>         
        <tr valign="top">
            <th scope="row"><?php echo esc_html( $this->optiontitle );?></th>
            <td>
                <input type="file" name="<?php echo esc_attr( $this->inputname );?>" id="<?php echo esc_attr( $this->inputid );?>"<?php if( isset( $this->disabled ) && $this->disabled === true ){ echo ' disabled';} ?>> 
            </td>
        </tr><?php 
    }      
    ...
    */
    public function input_menu_cronrepeat() {
        ...
        <tr valign="top">
            ...
            <td>            
                <select name="<?php echo esc_attr( $this->inputname );?>" id="<?php echo esc_attr( $this->inputid );?>"<?php if( $this->disabled ){ echo ' disabled';} ?>>
                    <option value="hourly" <?php if( $this->currentvalue == 'hourly' ){ echo $selected; }?>><?php _e( 'Hourly', 'wtgportalmanager' ); ?></option>
                    <option value="twicedaily" <?php if( $this->currentvalue == 'twicedaily' ){ echo $selected; }?>><?php _e( 'Twice Daily', 'wtgportalmanager' ); ?></option>
                    <option value="daily" <?php if( $this->currentvalue == 'daily' ){ echo $selected; }?>><?php _e( 'Daily', 'wtgportalmanager' ); ?></option>
                </select>                  
            </td>
        </tr>
        <!-- Option End --><?php     
    ...
    */
    public function input_menu_cronhooks() {
        ...
        <tr valign="top">
            <th scope="row"><label for="<?php echo esc_attr( $this->inputid ); ?>"><?php echo esc_html( $this->optiontitle ); ?></label></th>
            <td>            
                <select name="<?php echo esc_html( $this->inputname );?>" id="<?php echo esc_html( $this->inputid );?>"<?php if( $this->disabled ){ echo ' disabled';} ?>>
    ...
    */
    public function input_dateandtime() {
        ...
        if( !empty( $this->currentvalue ) && is_numeric( $this->currentvalue ) ) {
            $time_adj = esc_attr( $this->currentvalue );
        } else {        
    ...
    */
    public function hidden_basic( $form_id, $id, $name, $current_value ) {
        self::input( $form_id, 'hidden', $id, $name, __( 'Hidden Input ' . $id, 'wtgportalmanager' ), __( 'Hidden Input ' . $id, 'wtgportalmanager' ), true, $current_value, array(), array() ); 
    }
    ...
    */
    ...
    /**
    * Text input for password/hidden enries with basic 
    * parameters for common requirements. 
    * 
    * @author Ryan R. Bayne
    * @package WebTechGlobal WordPress Plugins
    * @since 0.0.1
    * @version 1.0
    */
    public function password_basic( $form_id, $id, $name, $title, $current_value = null, $required = false, $validation_array = array() ) {
        self::input( $form_id, 'password', $id, $name, $title, $title, $required, $current_value, array(), $validation_array );       
    }
    ...
    * @version 1.1
    * 
    * @deprecated use checkboxesgrouped_basic() or checkboxessingle_basic()
    */
    ...
    /**
    * Checkboxes single with incremented ID and name attribute.
    * 
    * @author Ryan R. Bayne
    * @package WebTechGlobal WordPress Plugins
    * @since 0.0.1
    * @version 1.1
    */
    public function checkboxessingle_basic( $form_id, $id, $name, $title, $items_array, $current_value, $required = false, $validation_array = array() ) {
        self::input( $form_id, 'checkboxessingle', $id, $name, $title, $title, $required, $current_value, array( 'itemsarray' => $items_array ), $validation_array );        
    } 
    
    /**
    * Checkboxes grouped in array.
    * 
    * @author Ryan R. Bayne
    * @package WebTechGlobal WordPress Plugins
    * @since 0.0.1
    * @version 1.1
    */
    public function checkboxesgrouped_basic( $form_id, $id, $name, $title, $items_array, $current_value, $required = false, $validation_array = array() ) {
        self::input( $form_id, 'checkboxesgrouped', $id, $name, $title, $title, $required, $current_value, array( 'itemsarray' => $items_array ), $validation_array );        
    }
