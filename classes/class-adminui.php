...
* @version 1.0 
*/
class WTGPORTALMANAGER_ADMINUI extends WTGPORTALMANAGER {     
    ...
    * @version 1.2
    * 
    * @todo this is still to be complete by adding arguments for each situation then changing the main action to "all" from "pluginpages", do not change "pluginpages" until arguments secure for each scenario.
    */
    ...
    */
    ...
    /**
    * Called by main WTGPORTALMANAGER() class using proper hook
    * 
    ...
    */
    public function add_dashboard_widgets() {
        ...
        wp_add_dashboard_widget(
             ...
             __( 'Test Widget', 'wtgportalmanager' ),// Title.
        ...
        foreach( $wtgportalmanager_menu_array as $key => $section_array ) {
            ...
            if( !isset( $wtgportalmanager_settings['widgetsettings'][ $section_array['name'] . 'dashboardwidgetsswitch'] ) ) {       
                continue;    
            } elseif( $wtgportalmanager_settings['widgetsettings'][ $section_array['name'] . 'dashboardwidgetsswitch'] !== 'enabled' ) {    
                continue;    
            }
            ...
            if( !isset( $wtgportalmanager_settings['widgetsettings'][ $section_array['name'] . 'widgetscapability'] ) ) {      
                continue;
            } elseif( !current_user_can( $wtgportalmanager_settings['widgetsettings'][ $section_array['name'] . 'widgetscapability'] ) ) {    
            ...
            $view_object = WTGPORTALMANAGER::load_class( 'WTGPORTALMANAGER_' . ucfirst( $section_array['name'] ) . '_View', $section_array['name'] . '.php', 'views' );
    ...
    */
    ...
    /**
    * Displays a table of wtgportalmanager_option records with ability to view their value or delete them
    ...
    */
    public function list_optionrecordtrace( $form = false, $size = 'Small', $optiontype = 'all' ){ 
        // first get all records that begin with wtgportalmanager_
        $wtgportalmanagerrecords_result = $this->DB->options_beginning_with( 'wtgportalmanager_' );
        $counter = 1;?>
            
            <script language="JavaScript">
            function wtgportalmanager_deleteoptions_checkboxtoggle(source) {
              checkboxes = document.getElementsByName( 'wtgportalmanager_deleteoptions_array[]' );
        ...
        if( $form){
            echo '<input type="checkbox" onClick="wtgportalmanager_deleteoptions_checkboxtoggle(this)" /> '. __( 'Select All Options', 'wtgportalmanager' ) .'<br/>';
        }
        
        $html = '';
                        
        foreach( $wtgportalmanagerrecords_result as $key => $option ){
            
            if( $form){
                $html = '<input type="checkbox" name="wtgportalmanager_deleteoptions_array[]" value="'.$option.'" />';
            }
    ...
    /**
    * Builds options for HTML form menu.
    * 
    * @author Ryan R. Bayne
    * @package WebTechGlobal WordPress Plugins
    * @version 1.2
    */
    public function screens_menuoptions( $current ){
        ...
        foreach( $wtgportalmanager_menu_array as $page_slug => $page_array ){ 
            ...
            $selected = '';
            if( $page_array['slug'] == $current){
                $selected = 'selected="selected"';    
            }             
            echo '<option value="'.$page_array['slug'].'" '.$selected.'>'.$page_array['pluginmenu'].'</option>'; 
    ...
    public function page_menuoptions( $current, $return = false ){
        ...
        foreach( $wtgportalmanager_menu_array as $page_slug => $page_array ){ 
            ...
            if( $return){
                return '<option value="'.$page_slug.'" '.$selected.'>'.$wtgportalmanager_menu_array[$page_slug]['title'].'</option>';
            }else{ 
                echo '<option value="'.$page_slug.'" '.$selected.'>'.$wtgportalmanager_menu_array[$page_slug]['title'].'</option>';
            }
        }    
    }
    ...
    * 
    * This status_box() function has been customized for WTG Portal Manager.
    * 
    * @author Ryan R. Bayne
    * @package WTG Portal Manager
    * @since 0.0.1
    ...
    */
    public function status_box(){
        ...
        $foot = __( 'Changes you make right now will only be applied to the current active portal.', 'wtgportalmanager' );
        ...
        // title
        $box_middle = '<h3>Active Portal: ' . WTGPORTALMANAGER_PROJECT_NAME . ' (' . WTGPORTALMANAGER_PROJECT_ID . ')</h3>';
 
        // content
        $box_middle .= self::info_area( 'Portal Description', WTGPORTALMANAGER_PROJECT_DESC );
        
        // foot
        $box_middle .= $foot;            

        echo $box_start . $box_middle . $box_end;
    ...
    */
    public function option_switch( $title, $name, $id, $current_value = 'nocurrent123', $onlabel = 'Enabled', $offlabel = 'Disabled', $default = 'enabled' ){
        ...
        <!-- Option Start -->
        <tr valign="top">
            <th scope="row"><?php _e( $title, 'wtgportalmanager' ); ?></th>
            <td>
    ...
    * @deprecated use class-forms.php      
    */
    public function option_text( $title, $name, $id, $current_value = '', $readonly = false, $class = 'wtgportalmanager_inputtext', $append_text = '', $left_field = false, $right_field_content = '', $required = false, $validation = false ){
    ...
    */
    public function option_text_simple( $title, $nameandid, $current_value = '', $required = false, $validation = false ){
        $this->option_text( $title, $nameandid, $nameandid, $current_value, false, 'wtgportalmanager_inputtext', '', false, '', $required, $validation );
    }
    ...
    * @version 1.1
    * 
    * @todo add option to Screen Options tab (WP Core admin feature) to 
    * display the intro_box after it is hidden.
    *     
    * @param mixed $title
    * @param mixed $intro
    * @param mixed $info_area
    * @param mixed $info_area_title
    * @param mixed $info_area_content
    * @param mixed $footer
    * @param mixed $dismissable_id if false will not show dismiss button
    */
    public function intro_box( $title, $intro, $info_area = false, $info_area_title = '', $info_area_content = '', $footer = false, $dismissable_id = false ){
        global $multitool_settings,$current_user;
    ...
    */
    public function metabox_icons( $form_id, $trash_icon = false ){
        ...
        if( WTGPORTALMANAGER::is_dashboard() ) {
            return false;    
        }
    ...
    public function panel_trash_icon( $form_name ){?>
        <a href="<?php echo $form_name;?>"><img src="<?php echo WTGPORTALMANAGER_IMAGES_URL . 'trash-icon.gif';?>" alt="<?php _e( 'Reset form' );?>" title="<?php _e( 'Reset form' );?>"></a><?php 
    }
    
    public function panel_video_icon( $youtubeid ){
        if( $youtubeid){
            echo '<a href="https://www.youtube.com/watch?v=' . $youtubeid . '" target="_blank"><img src="' . WTGPORTALMANAGER_IMAGES_URL . 'video-icon.gif" alt="' . __( 'View video on YouTube' ) . '" title="' . __( 'View video on YouTube' ) . '"></a>';
        }
    }
    ...
    */
    public function helpicon( $url){
        return '<a href="'.$url.'" title="'. __( 'View help for this on the plugins website.' ) .'" target="_blank"><img src="'. WTGPORTALMANAGER_IMAGES_URL . 'help-icon.png" alt="'. __( 'View help for this on the plugins website' ) .'"></a>';
    }    
    public function videoicon( $url){
        return '<a href="'.$url.'" title="'. __( 'View a video about this feature.' ) .'" target="_blank"><img src="'. WTGPORTALMANAGER_IMAGES_URL . 'video-icon.png" alt="'. __( 'Video icon, click to open a video' ) .'"></a>';
    }     
    public function trashicon( $url){
        return '<a href="'.$url.'" title="'. __( 'Delete or hide the current feature.' ) .'" target="_blank"><img src="'. WTGPORTALMANAGER_IMAGES_URL . 'trash-icon.png" alt="'. __( 'Trash icon, click to delete or hide the current item' ) .'"></a>';
    }    
    public function discussicon( $url){
        return '<a href="'.$url.'" title="'. __( 'Discuss this feature on the WebTechGlobal forum.' ) .'" target="_blank"><img src="'. WTGPORTALMANAGER_IMAGES_URL . 'chat-icon.png" alt="'. __( 'Chat icon which you can click to discuss this feature on the WebTechGlobal forum.' ) .'"></a>';
    }  
    ...
    */
    public function postbox_content_header( $form_title, $form_id, $introduction = false, $trash_icon = false, $uploader = false ) {?>
        <p>
            ...
            <?php if( $introduction !== false && !empty( $introduction) ){echo "<p class=\"wtgportalmanager_boxes_introtext\">". $introduction ."</p>";}?>
    ...
    */          
    public function box_information_icon( $page_name, $view_name, $form_id ) {
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formreadmoreurl' ] ) ){
            $all_help_content .= '<p>';
            $all_help_content .= __( 'You are welcome to visit the', 'wtgportalmanager' ) . ' ';
            $all_help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formreadmoreurl' ] . '"';
            $all_help_content .= 'title="' . __( 'Visit the WTG Portal Manager website and read more about', 'wtgportalmanager' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '"';
            $all_help_content .= 'target="_blank"';
            $all_help_content .= '>';
            $all_help_content .= __( 'WTG Portal Manager Website', 'wtgportalmanager' ) . '</a> ' . __( 'to read more about', 'wtgportalmanager' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ];           
            $all_help_content .= '.</p>';
        }  
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formvideourl' ] ) ){
            $all_help_content .= '<p>';
            $all_help_content .= __( 'There is a', 'wtgportalmanager' ) . ' ';
            $all_help_content .= '<a href="' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formvideourl' ] . '"';
            $all_help_content .= 'title="' . __( 'Go to YouTube and watch a video about', 'wtgportalmanager' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '"';
            $all_help_content .= 'target="_blank"';
            $all_help_content .= '>';            
            $all_help_content .= __( 'YouTube Video', 'wtgportalmanager' ) . '</a> ' . __( 'about', 'wtgportalmanager' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ];           
            $all_help_content .= '.</p>';
        }
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formdiscussurl' ] ) ){
            ...
            $all_help_content .= 'title="' . __( 'Visit the WebTechGlobal forum to discuss', 'wtgportalmanager' ) . ' ' . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '"';
            $all_help_content .= 'target="_blank"';
            $all_help_content .= '>';            
            $all_help_content .= $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '</a> ' . __( 'on the WebTechGlobal Forum', 'wtgportalmanager' );           
            $all_help_content .= '.</p>';
        }        
        ...
        if( isset( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'options' ] ) ){
            foreach( $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'options' ] as $option_id => $option_array ){
                ...
                if( isset( $option_array['optionurl'] ) ){
                    ...
                    $all_help_content .= ' title="' . __( 'Read More about', 'wtgportalmanager' )  . ' ' . $option_array['optiontitle'] . '"';
                    $all_help_content .= ' target="_blank">';
                    $all_help_content .= __( 'Read More', 'wtgportalmanager' ) . '</a>';      
                }
      
                if( isset( $option_array['optionvideourl'] ) ){
                    $all_help_content .= ' - <a href="' . $option_array['optionvideourl'] . '"';
                    $all_help_content .= ' title="' . __( 'Watch a video about', 'wtgportalmanager' )  . ' ' . $option_array['optiontitle'] . '"';
                    $all_help_content .= ' target="_blank">';
                    $all_help_content .= __( 'Video', 'wtgportalmanager' ) . '</a>';      
                }
        ...
        $all_help_content .= __( 'Help for ', 'wtgportalmanager' ) . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '">';
        $all_help_content .= '<img src="' . WTGPORTALMANAGER_IMAGES_URL . 'info-icon.gif" alt="';
        $all_help_content .= __( 'Click icon to view help for ', 'wtgportalmanager' ) . $help_array[ $page_name ][ $view_name ][ 'forms' ][ $form_id ][ 'formtitle' ] . '"></a>';
    ...
    */
    public function notice_return( $type, $size, $title = false, $message = 'no message has been set', $helpurl = false, $forcewtgstyle = false ){
        ...
        if( $forcewtgstyle === false ) {
            if( isset( $wtgportalmanager_settings['noticesettings']['wpcorestyle'] ) && $wtgportalmanager_settings['noticesettings']['wpcorestyle'] == 'enabled' ) {
    ...
    * 
    * @deprecated use wtgportalmanager_n_postresult as of 13th February 2013
    */
    ...
    /**
    * New Notice Function (replacing WTGPORTALMANAGER_Notice::notice_depreciated() )
    ...
    */
    public function update_persistentnotifications_array( $notifications_array ){
        return update_option( 'wtgportalmanager_notifications',maybe_serialize( $notifications_array ) );    
    }
    ...
    */
    public function notice_depreciated( $message, $type = 'success', $size = 'Extra', $title = false, $helpurl = 'www.webtechglobal.co.uk', $output_type = 'echo', $persistent = false, $clickable = false, $user_type = false ){
        ...
        if( is_admin() || $output_type == 'public' ){
            ...
            if( $helpurl != false && $helpurl != '' && $helpurl != 'http://www.wtgportalmanager.com/support' ){$clickable = false;}
    ...
    */
    ...
    public function add_admin_footer_text() {
        // show admin footer message (only on pages of WTG Portal Manager)
        add_filter( 'admin_footer_text', array( $this, '_admin_footer_text' ) );
    }
    
    /**
     * Add a WTG Portal Manager "Thank You" message to the admin footer content
     *
     * @since 0.0.1
     *
    ...
    */
    public function register_input_validation( $input_title, $input_name, $input_id, $input_validation ) {
        ...
        $form_val_arr = get_option( 'wtgportalmanager_formvalidation' );
        ...
        update_option( 'wtgportalmanager_formvalidation', $form_val_arr );
    }
    ...
    */
    public function metabox_nocurrentproject() {
        ...
        the one that was active.', 'wtgportalmanager' );
        echo '</p>';
    }
    ...
    * @param mixed $values - begin with & followed by values
    * 
    * @deprecated this method has been moved to the WTGPORTALMANAGER_ADMINUI class
    */
...
}// class WTGPORTALMANAGER_ADMINUI 
...
* @version 1.0
*/
class WTGPORTALMANAGER_Pointers {
